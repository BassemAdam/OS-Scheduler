                ...
                while (recievedProcess.id != -1)
                {
                    ...
                    if (recievedProcess.id != -1)
                    {
                        ...
                        if (currentlyRunningProcess.id == -1) // no running process
                        {
                            ...
                            lastRunningProcessId = currentlyRunningProcess.id;
                        }
                        else // running process and recieved a process
                        {
                            ...
                            if (currentlyRunningProcess.id == lastRunningProcessId)
                            {
                                if (kill(currentlyRunningProcess.pid, decrementTime) == -1) 
                                {
                                    printf("Error: failed to send SIGUSR2 to process %d\n", currentlyRunningProcess.pid);
                                }
                            }
                        }
